...
country:string = "US";
verifyAddress(state:string ,city:string,address1:string, zipcode:number):Promise<object>{
  return this.http.get<object>(`https://api.ekata.com/3.1/location_intel?api_key=${this.id}&street_line_1=${address1}&city=${city}&state_code=${state}&postal_code=${zipcode}&country_code=${this.country}`).toPromise();
}
}
